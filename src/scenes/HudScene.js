...
const COMPACT_SELECTION_DETAILS_Y_PAD = 38;  // padding from panelY for compact selection details Y
...
// ── Top-bar accent line Y helper (extracted from create/handleResize) ───
const topBarAccentLineY = (layout) => layout.topBarY - ACCENT_LINE_Y_OFFSET;

// ── Bottom-bar top-Y helper (single source of truth for baseline) ──────────
const bottomBarTopY = (width, height) => height - getLayout(width, height).bottomBarHeight;

// ── Selection panel Y helper (extracted from create/handleResize) ─────────
const selectionPanelY = (width, height) => {
  const bottomBarY = bottomBarTopY(width, height);
  if (layout.compact) {
...
const computeTextPositions = (width, height) => {
  ...
  if (layout.compact) {
    return {
      ...
      selectionDetailsY: panelY + COMPACT_SELECTION_DETAILS_Y_PAD,
...
export default class HudScene extends Phaser.Scene {
  ...
  create() {
    ...
    this.accentLine = this.add.rectangle(width / 2, topBarAccentLineY(this.layout), width, ACCENT_LINE_HEIGHT, raceColors[initialRaceId] ?? 0x1d4ed8, 0.7).setOrigin(0.5);
  ...
  handleResize(gameSize) {
    ...
    this.topBarBorder.setPosition(width / 2, topBarBorderCenterY(this.layout));
    this.topBarBorder.width = width;

    this.accentLine.setPosition(width / 2, topBarAccentLineY(this.layout));
